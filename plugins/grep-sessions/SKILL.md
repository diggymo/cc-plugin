...
3. `ccs` が見つからない（command not found）場合は、aliasでccsコマンドを設定した上で、`${CLAUDE_SKILL_DIR}/search.py` で直接実行する
...
以下のalias設定を行う。ユーザーの環境に合わせて`~/.zshrc` など適切なファイルの末尾に書き込む。

```sh
alias ccs='python3 ${CLAUDE_SKILL_DIR}/search.py'
```

その後、今回は以下のコマンドで直接実行する:

...
claude plugin install google-drive-markdown-editor
```
...
### [google-drive-markdown-editor](./plugins/google-drive-markdown-editor/)

Google Drive の .md ファイルをブラウザで編集できる Web アプリプラグイン。

- Google Drive の「アプリで開く」から直接起動可能
- 左にエディタ（CodeMirror）、右にリアルタイムプレビューの分割画面
- 入力停止 2 秒後に Google Drive へ自動保存
- Hono バックエンドを AWS Lambda にデプロイして使用
